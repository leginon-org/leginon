...
results = leginondata.InstrumentData(hostname=hostname,name='DE20').query(results=1)
if not results:
	print("ERROR: incorrect hostname....")
	r = leginondata.InstrumentData(name='DE20').query(results=1)
	if r:
		print(("  Try %s instead" % r[0]['hostname']))
	else:
		print("  No DE20 camera found")
	sys.exit()

sourcecam = results[0]
#sourcecam = leginondata.InstrumentData(hostname=hostname,name='GatanK2Super').query(results=1)[0]
destcam = leginondata.InstrumentData(hostname=hostname,name='DE20c').query(results=1)[0]
pixelsize_scale = 0.5
