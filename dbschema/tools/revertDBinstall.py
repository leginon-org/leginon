...
if __name__ == "__main__":
	...
	checkout_version = eval(input('Revert to checkout version, for example, 2.1 -->'))
	if checkout_version != 'trunk':
		try:
			list(map((lambda x:int(x)),checkout_version.split('.')[:2]))
		except:
			print("valid versions are 'trunk', '2.1', or '2.1.2' etc")
			raise
	checkout_revision = int(eval(input('Revert to checkout revision, for example, 16500 -->')))
