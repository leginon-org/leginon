...
if ($_POST['process']) {
	...
	runImagicReferenceFreeAlignment();
}
else jobform();
...
//	$imagicnorefruns=count($imagicnorefids);
	...
	processing_header("IMAGIC Reference Free Alignment","IMAGIC Reference Free Alignment",$javafunc);
	...
	$runid = ($_POST['runid']) ? $_POST['runid'] : 'align'.($imagicnorefruns+1);
	...
	echo docpop('runid','<b>Reference Free Run Name:</b>');
	...
	echo "<BR/>\n";
	echo "<BR/>\n";
	echo docpop('descr','<b>Description of IMAGIC Reference Free Alignment:</b>');
	echo "<BR/>\n";
...
function runImagicReferenceFreeAlignment($extra=false)	{
	$expId=$_GET['expId'];
	$runid=$_POST['runid'];
	$outdir=$_POST['outdir'];
	...
	$command = "$outdir/$runid/{$runid}_imagicReferenceFreeAlignment.job";
	...
	// write to jobfile
	$jobfile = "{$runid}_imagicReferenceFreeAlignment.job";
	...
	if ($_POST['process']=="run imagic") {
		...
		$sub = submitAppionJob($command,$outdir,$runid,$expId,'runImagicReferenceFreeAlignment');
		// if errors:
