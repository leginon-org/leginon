...
$corr_gif_files = glob("$outdir/$runname/media/corrplots/series".sprintf('%04d',$tiltseries).sprintf('%02d',$iter-1)."*.gif");
$tilt_gif_files = glob("$outdir/$runname/media/tiltseries/s*.gif");
$tilt_vid_files = glob("$outdir/$runname/media/tiltseries/series".sprintf('%04d',$tiltseries).sprintf('%02d',$iter-1).".{mp4,ogv,webm}",GLOB_BRACE);
...
if (isset($tilt_gif_files[0])) {
	$html .= '<center><img src="'.$tilt_gif.'" alt="correlations" /></center>';
        ...
        $html .= "<center><b>Depiction Tilt Series Video for Refinement Iteration ".$iter." either failed to generate or is still processing</b></center>";
}
...
if (isset($rec_gif_files[0])) {
	$html .= '<center><img src="'.$rec_gif.'" alt="correlations" /></center>';
        ...
        $html .= "<center><b>Depiction Reconstruction Video for Coarse Alignment either failed to generate or is still processing</b></center>";
}
