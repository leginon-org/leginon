...
/******* IMAGIC terms ********/
		...
		'protomo_filter' : 'Preprocessing filter. Options are "median" or "gauss" . If "gauss" is chosen, the radius of the Gaussian will be 1/2 of the Filter Window Size (type: string).',
		'protomo_corr_mode' : 'Correlation mode, standard (xcf), mutual (mcf), phase only (pcf), phase doubled (dbl).<br><br>xcf: Conventional cross-correlation.<br><br>mcf: Mutual correlation function. mcf is less biased towards strong low-frequency components.<br><br>pcf: Phase only correlation function. pcf ignores the amplitude of the Fourier coefficients and only aligns based on phase. pcf might be useful in Refinements because it requires accurate geometric parameters as a starting point.<br><br>dbl: Phase doubled correlation. dbl doubles the peak shift allowing for more accurate alignments especially on tilt-series taken with poor imaging conditions.<br><br>Note: ccc values outputed to the command window by Protomo during alignment are normalized to xcf ccc values; mcf, pcf, and dbl ccc values are not normalized to 1 (type: string selection).',
		...
		'protomo_show_window_size' : 'Select \'true\' to have the Window Size, which is the search area, displayed on the reconstruction video on the summary page. This option is particularly useful for diagnosing alignment issues.',
		'protomo_keep_recons' : 'Keep premilinary reconstructions used for generating a reconstruction video (type: true/false).',
		...
		'protomo_orientation' : 'Include orientation angles in refinement. It is recommended to leave this set to true (type: true/false).',		
		'protomo_azimuth' : 'Include tilt azimuth in refinement. It is recommended to leave this set to true (type: true/false).',		
		...
		'protomo_rotation' : 'Include in-plane rotations in refinement. It is recommended to leave this set to true (type: true/false).',
		...
		'windowparam_info' : 'Protomo will use the following parameters during alignment.',
		...
		'initialtiltimageinfo_coarse' : 'This is the tilt-series before any alignment, but including shifts and rotations as recorded in the database.',
		'tiltimageinfo_coarse' : 'These are the Raw Images translated and rotated according to the Coarse Alignment. Grey borders correspond to translations. Black borders correspond to rotations. Excluded images are included in this depiction. To view the initial tilt-series before alignment, click on the \'View Initial Tilt-Series\' button above this image.',
		'reconimageinfo_coarse' : 'This is a Z-slice through of the binned reconstruction for depiction. You should be able to see large, high contrast objects present in your tomogram. All objects in your zero-degree tilt image should be visible somewhere in the middle Z-slices. If they are not, consider increasing the Thickness and re-running Coarse Alignment.<br><br>If you set the Show Window Size in Reconstruction Videos to true, in the middle half of the Z-slices you will see a box the size of the requested Window Size. The volume enclosed by this box is approximately the volume used to perform Coarse Alignment. It is best to choose a Window Size that encloses a large amount of the objects in your tomogram in the x- and y-directions, and to choose a Thickness that encloses all of those objects that you wish to align on.',
		'recon_lowpass' : 'Apply a lowpass filter to images before reconstruction by back-projection?<br><br>If you are planning on performing single particle tomography on the reculting tomogram, set this to False. If instead you are planning to use the tomogram for segmenting or visualization purposes, set this to True and set the lowpass filters to the resolution you are hoping to be able to segment from or visualize. (type: True/False).',
		...
		'prep_info' : 'This step prepares tilt-series for processing. A directory for each tilt-series is made, raw images are linked/copied, and leginon database is queried to generate a geometry .tlt file.',
		'coarse_info' : 'This step processes tilt-series through a coarse alignment based on the parameters in the param file provided by the user. Correlation peak videos, initial tilt-series videos, coarse aligned tilt-series videos, and reconstruction videos will be generated for depiction. Tilt-series need to have been prepared before this step. Leginon database is not queried during this step.',
		'refine_info' : 'This step processes tilt-series through refinements based on the parameters in the param file provided by the user. Correlation peak videos and correlation plots will be generated for depiction. Tilt-series videos and reconstruction videos will be made if requested (see Depiction Parameters below). Tilt-series need to have been coarsely aligned, either through Appion or manually, before this step. Leginon database is not queried during this step.',
		'recon_info' : 'This step processes tilt-series through reconstructions based on the parameters in the param file provided by the user. No depictions will be made. Tilt-series need to have been aligned (either through Appion or manually) before this step. Leginon database is not queried during this step.',
		'recon_info2' : 'Weighted back-projection reconstruction. Maximum resolution of the resulting reconstruction will at best be 2*pixelsize*sampling. For single particle tomography (SPT), do not apply a lowpass filter. For segmentation, depiction, or other non-SPT purposes, consider applying a lowpass filter to reduce noise and reconstruction artifacts.',
		...
		'ctf_info' : 'CTF Correction requires that tilt-series images have already had their defocus estimated through the Appion interface such that the defocus and defocus confidence values have been inserted into the Leginon Database. The CTF correction algorithm first removes the low confidence defocus values, then removes defocus values that exceed one standard deviation from the tilt-series mean, and finally replaces those values with linearly interpolated values between the remaining high confidence defocus values in the tilt-series.<br><br>CTF correction is performed strip-by-strip using ctfphaseflip from the IMOD package. Please use the citation below in publications.',
		'auto_info' : 'Fully Automated mode processes tilt-series in batch through starting at the File Preparation, Coarse Alignment, or Refinement steps. In this mode each round begins with the best iteration from the previous Round based on the CCMS values. If convergence is reached at a binning of 1 or 2, then a reconstruction is made and is placed in a common folder. If convergence is not reached by the end of Round 5, a further 3 Rounds will be run with scaling and/or tilt axis elevation included in the refinement.<br><br>A restricted set of parameters are given. Default values ensure an exhaustive tilt image geometry search.',
		'quality_assessment' : 'This plot shows the sum of the average correction factors and their standard deviations for each iteration. Generally lower is better.',
		'quality_assessment2' : 'Protomo refines geometric parameters by diagonalizing correction geometric matricies. This is done numerically, so the solution and thus the correction is not exact. To estimate the quality of a tilt-series, the correction factors for x and y are averaged (with repect to 1.0; a Protomo convention) and the standard deviation of each is calculated. Plotted here is the sum of the average correction factors and the standard deviations:<br><br>CCMS = Avg(corrx)+Avg(corry)+Stdev(corrx)+Stdev(corry)<br><br>A candidate iteration for a well-aligned tilt-series will have a CCMS value below 0.02, ideally below 0.015, at a binning of 1 or 2.<br><br>Note that the CCMS value for each iteration is relative to the alignment from the previous iteration, similar to projection matching in single particle cryo-EM; the geometric model from the previous iteration is used to create projection images to which images are aligned to for the present iteration.',
...
**/
	...
	'protomo' : {
		...
		'region_x' : 'Pixels in x to use for region matching. Default: 60% Raw Image size. This number will be binned by the requested sampling before alignment. It is recommended that you make the Window Size/Sampling a number that only has prime factors of 2, 3, or 5 (type: int).',
		'region_y' : 'Pixels in y to use for region matching. Default: 60% Raw Image size. This number will be binned by the requested sampling before alignment. It is recommended that you make the Window Size/Sampling a number that only has prime factors of 2, 3, or 5 (type: int).',
		'r1_region_x' : 'Pixels in x to use for region matching for Round 1. Default: 50% Raw Image size. This number will be binned by the requested sampling before alignment. It is recommended that you make the Window Size/Sampling a multiple of 2, 3, or 5 (type: int).',
		'r2_region_x' : 'Pixels in x to use for region matching for Round 2. Default: 50% Raw Image size. This number will be binned by the requested sampling before alignment. It is recommended that you make the Window Size/Sampling a multiple of 2, 3, or 5 (type: int).',
		'r3_region_x' : 'Pixels in x to use for region matching for Round 3. Default: 50% Raw Image size. This number will be binned by the requested sampling before alignment. It is recommended that you make the Window Size/Sampling a multiple of 2, 3, or 5 (type: int).',
		'r4_region_x' : 'Pixels in x to use for region matching for Round 4. Default: 50% Raw Image size. This number will be binned by the requested sampling before alignment. It is recommended that you make the Window Size/Sampling a multiple of 2, 3, or 5 (type: int).',
		'r5_region_x' : 'Pixels in x to use for region matching for Round 5. Default: 50% Raw Image size. This number will be binned by the requested sampling before alignment. It is recommended that you make the Window Size/Sampling a multiple of 2, 3, or 5 (type: int).',
		'r1_region_y' : 'Pixels in y to use for region matching for Round 1. Default: 50% Raw Image size. This number will be binned by the requested sampling before alignment. It is recommended that you make the Window Size/Sampling a multiple of 2, 3, or 5 (type: int).',
		'r2_region_y' : 'Pixels in y to use for region matching for Round 2. Default: 50% Raw Image size. This number will be binned by the requested sampling before alignment. It is recommended that you make the Window Size/Sampling a multiple of 2, 3, or 5 (type: int).',
		'r3_region_y' : 'Pixels in y to use for region matching for Round 3. Default: 50% Raw Image size. This number will be binned by the requested sampling before alignment. It is recommended that you make the Window Size/Sampling a multiple of 2, 3, or 5 (type: int).',
		'r4_region_y' : 'Pixels in y to use for region matching for Round 4. Default: 50% Raw Image size. This number will be binned by the requested sampling before alignment. It is recommended that you make the Window Size/Sampling a multiple of 2, 3, or 5 (type: int).',
		'r5_region_y' : 'Pixels in y to use for region matching for Round 5. Default: 50% Raw Image size. This number will be binned by the requested sampling before alignment. It is recommended that you make the Window Size/Sampling a multiple of 2, 3, or 5 (type: int).',
		'sampling' : 'Sampling rate of Raw Images (type: real >= 1 and <= 15).',
		'r1_sampling' : 'Sampling rate of Raw Images for Round 1. It is recommended to keep this value 10 or below. Multiple Refinement iterations performed with a Sampling of 2 or 1 is highly recommended (type: real >= 1 and <= 15).',
		'r2_sampling' : 'Sampling rate of Raw Images for Round 2. It is recommended to keep this value 10 or below. Multiple Refinement iterations performed with a Sampling of 2 or 1 is highly recommended (type: real >= 1 and <= 15).',
		'r3_sampling' : 'Sampling rate of Raw Images for Round 3. It is recommended to keep this value 10 or below. Multiple Refinement iterations performed with a Sampling of 2 or 1 is highly recommended (type: real >= 1 and <= 15).',
		'r4_sampling' : 'Sampling rate of Raw Images for Round 4. It is recommended to keep this value 10 or below. Multiple Refinement iterations performed with a Sampling of 2 or 1 is highly recommended (type: real >= 1 and <= 15).',
		'r5_sampling' : 'Sampling rate of Raw Images for Round 5. It is recommended to keep this value 10 or below. Multiple Refinement iterations performed with a Sampling of 2 or 1 is highly recommended (type: real >= 1 and <= 15).',
		'auto_r1_sampling' : 'Sampling rate of Raw Images for Round 1. It is recommended to keep this value 10 or below. Multiple Refinement iterations performed with a Sampling of 2 or 1 is highly recommended (type: real >= 1 and <= 15).',
		'auto_r2_sampling' : 'Sampling rate of Raw Images for Round 2. It is recommended to keep this value 10 or below. Multiple Refinement iterations performed with a Sampling of 2 or 1 is highly recommended (type: real >= 1 and <= 15).',
		'auto_r3_sampling' : 'Sampling rate of Raw Images for Round 3. It is recommended to keep this value 10 or below. Multiple Refinement iterations performed with a Sampling of 2 or 1 is highly recommended (type: real >= 1 and <= 15).',
		'auto_r4_sampling' : 'Sampling rate of Raw Images for Round 4. It is recommended to keep this value 10 or below. Multiple Refinement iterations performed with a Sampling of 2 or 1 is highly recommended (type: real >= 1 and <= 15).',
		'auto_r5_sampling' : 'Sampling rate of Raw Images for Round 5. It is recommended to keep this value 10 or below. Multiple Refinement iterations performed with a Sampling of 2 or 1 is highly recommended (type: real >= 1 and <= 15).',
		'auto_r6_sampling' : 'Sampling rate of Raw Images for Round 6. It is recommended to keep this value 10 or below. Multiple Refinement iterations performed with a Sampling of 2 or 1 is highly recommended (type: real >= 1 and <= 15).',
		'auto_r7_sampling' : 'Sampling rate of Raw Images for Round 7. It is recommended to keep this value 10 or below. Multiple Refinement iterations performed with a Sampling of 2 or 1 is highly recommended (type: real >= 1 and <= 15).',
		'auto_r8_sampling' : 'Sampling rate of Raw Images for Round 8. It is recommended to keep this value 10 or below. Multiple Refinement iterations performed with a Sampling of 2 or 1 is highly recommended (type: real >= 1 and <= 15).',
		...
		'lowpass_diameter_x' : 'Lowpass filter (x), in angstroms. This will filter out frequencies higher than the chosen value. Default is pixelsize*75.<br/><br/>This is a critical parameter. If you don\'t filter out frequencies higher than the highest resolvable structure of your specimen, Protomo might align to noise (type: float).',
		'lowpass_diameter_y' : 'Lowpass filter (y), in angstroms. This will filter out frequencies higher than the chosen value. Default is pixelsize*75.<br/><br/>This is a critical parameter. If you don\'t filter out frequencies higher than the highest resolvable structure of your specimen, Protomo might align to noise (type: float).',	
		...
		'mask_apod_x' : 'Apodization (x) for rectangular and ellipsoidal masks, in pixels.<br/><br/>Apodization is a set of bandpass filters applied to soften edges fourier space so that edges images are not aligned to edges and so that there are no edge ripples from fourier space inversion that would otherwise add ordered noise to the real space images (type: float).',
		...
		'r1_mask_width_x' : 'Rectangular mask size (x) for Round 1, in pixels. It is recommended that this value be less than or equal to Window Size (x) divided by Sampling (type: float).',
		'r2_mask_width_x' : 'Rectangular mask size (x) for Round 2, in pixels. It is recommended that this value be less than or equal to Window Size (x) divided by Sampling (type: float).',
		'r3_mask_width_x' : 'Rectangular mask size (x) for Round 3, in pixels. It is recommended that this value be less than or equal to Window Size (x) divided by Sampling (type: float).',
		'r4_mask_width_x' : 'Rectangular mask size (x) for Round 4, in pixels. It is recommended that this value be less than or equal to Window Size (x) divided by Sampling (type: float).',
		'r5_mask_width_x' : 'Rectangular mask size (x) for Round 5, in pixels. It is recommended that this value be less than or equal to Window Size (x) divided by Sampling (type: float).',
		'r1_mask_width_y' : 'Rectangular mask size (y) for Round 1, in pixels. It is recommended that this value be less than or equal to Window Size (y) divided by Sampling (type: float).',	
		'r2_mask_width_y' : 'Rectangular mask size (y) for Round 2, in pixels. It is recommended that this value be less than or equal to Window Size (y) divided by Sampling (type: float).',	
		'r3_mask_width_y' : 'Rectangular mask size (y) for Round 3, in pixels. It is recommended that this value be less than or equal to Window Size (y) divided by Sampling (type: float).',	
		'r4_mask_width_y' : 'Rectangular mask size (y) for Round 4, in pixels. It is recommended that this value be less than or equal to Window Size (y) divided by Sampling (type: float).',	
		'r5_mask_width_y' : 'Rectangular mask size (y) for Round 5, in pixels. It is recommended that this value be less than or equal to Window Size (y) divided by Sampling (type: float).',			
		'r1_lowpass_diameter_x' : 'Lowpass filter (x) for Round 1, in angstroms. This will filter out frequencies higher than the chosen value. If your correlation peaks are visible throughout the entire tilt-series, you may wish to lower the lowpass filter to include more high resolution information for refinement.<br/><br/>This is a critical parameter. If you don\'t filter out frequencies higher than the highest resolvable structure of your specimen, Protomo might align to noise. However, even with sub-pixel accuracy, if you don\'t make this value low enough this might limit the resolution of subvolume averaging (type: float).',			
		...
		'r1_mask_apod_x' : 'Apodization (x) for rectangular and ellipsoidal masks for Round 1, in pixels (type: float).',		
		'r2_mask_apod_x' : 'Apodization (x) for rectangular and ellipsoidal masks for Round 2, in pixels (type: float).',	
		'r3_mask_apod_x' : 'Apodization (x) for rectangular and ellipsoidal masks for Round 3, in pixels (type: float).',	
		'r4_mask_apod_x' : 'Apodization (x) for rectangular and ellipsoidal masks for Round 4, in pixels (type: float).',	
		'r5_mask_apod_x' : 'Apodization (x) for rectangular and ellipsoidal masks for Round 5, in pixels (type: float).',	
		'r1_mask_apod_y' : 'Apodization (y) for rectangular and ellipsoidal masks for Round 1, in pixels (type: float).',	
		'r2_mask_apod_y' : 'Apodization (y) for rectangular and ellipsoidal masks for Round 2, in pixels (type: float).',	
		'r3_mask_apod_y' : 'Apodization (y) for rectangular and ellipsoidal masks for Round 3, in pixels (type: float).',	
		'r4_mask_apod_y' : 'Apodization (y) for rectangular and ellipsoidal masks for Round 4, in pixels (type: float).',	
		'r5_mask_apod_y' : 'Apodization (y) for rectangular and ellipsoidal masks for Round 5, in pixels (type: float).',		
		...
		'raw_path' : 'Full path to raw image files. Use if Appion doesn\'t find raw images automatically. If this option is used, then you will also need to place your own \'series%04d<tiltseriesnumber>.tlt\' file in the \'Run name\' directory if Leginon cannot query the Raw Image metadata. It is recommended, however, that you instead upload your tilt-series to the Leginon/Appion database and then process your tilt-series normally (type: string).',		
		...
		'auto_r1_iters' : 'Number of alignment and geometry refinement iterations for Round 1 (type: int).',
		'auto_r2_iters' : 'Number of alignment and geometry refinement iterations for Round 2 (type: int).',
		'auto_r3_iters' : 'Number of alignment and geometry refinement iterations for Round 3 (type: int).',
		'auto_r4_iters' : 'Number of alignment and geometry refinement iterations for Round 4 (type: int).',
		'auto_r5_iters' : 'Number of alignment and geometry refinement iterations for Round 5 (type: int).',
		'auto_r6_iters' : 'Number of alignment and geometry refinement iterations for Round 6. If convergence is not met by Round 5, up to three more Rounds will be run with tilt axis elevation and scaling refinement turned on (type: int).',
		'auto_r7_iters' : 'Number of alignment and geometry refinement iterations for Round 7. If convergence is not met by Round 5, up to three more Rounds will be run with tilt axis elevation and scaling refinement turned on (type: int).',
		'auto_r8_iters' : 'Number of alignment and geometry refinement iterations for Round 8. If convergence is not met by Round 5, up to three more Rounds will be run with tilt axis elevation and scaling refinement turned on (type: int).',
		'auto_convergence' : 'Stop refinement once CCMS value is less than or equal to this value. Set to zero to perform all iterations (type: float).',
		...
		'exclude_images' : 'Exclude images from the tilt-series. Eg. 1,2,5-7. Only use numbers, commas, and hyphens (type: int selection).',		
		'border' : 'Width of area at the image edge to exclude from image statistics, in pixels (type: int > 0).',		
		'clip_low' : 'Lower threshold specified as a multiple of the standard deviation of the pixel density values per Raw Image (type: float).',		
		'clip_high' : 'Upper threshold specified as a multiple of the standard deviation of the pixel density values per Raw Image (type: float).',
		'thr_low' : 'Lower threshold specified as density values. Alternative to \'Lower Clip\' parameter (type: float).',		
		'thr_high' : 'Upper threshold specified as density values Alternative to \'Upper Clip\' parameter (type: float).',
		...
		'kernel_x' : 'Filter window size (x) of the Preprocessing Filter, in pixels (type: int).',			
		'kernel_y' : 'Filter window size (y) of the Preprocessing Filter, in pixels (type: int).',	
		'r1_kernel_x' : 'Filter window size (x) of the Preprocessing Filter for Round 1, in pixels (type: int).',
		'r2_kernel_x' : 'Filter window size (x) of the Preprocessing Filter for Round 2, in pixels (type: int).',
		'r3_kernel_x' : 'Filter window size (x) of the Preprocessing Filter for Round 3, in pixels (type: int).',
		'r4_kernel_x' : 'Filter window size (x) of the Preprocessing Filter for Round 4, in pixels (type: int).',
		'r5_kernel_x' : 'Filter window size (x) of the Preprocessing Filter for Round 5, in pixels (type: int).',
		'r1_kernel_y' : 'Filter window size (y) of the Preprocessing Filter for Round 1, in pixels (type: int).',
		'r2_kernel_y' : 'Filter window size (y) of the Preprocessing Filter for Round 2, in pixels (type: int).',
		'r3_kernel_y' : 'Filter window size (y) of the Preprocessing Filter for Round 3, in pixels (type: int).',
		'r4_kernel_y' : 'Filter window size (y) of the Preprocessing Filter for Round 4, in pixels (type: int).',
		'r5_kernel_y' : 'Filter window size (y) of the Preprocessing Filter for Round 5, in pixels (type: int).',
		'window_area' : 'Fraction of extracted area that must lie within the source image. Real value between 0 and 1 (type: float).',
		'grow' : 'Grow the selected regions in the binary mask by the specified number of pixels. The first preprocessing step defines the binary mask area (type: int).',
		...
		'correlation_size_x' : 'Size (x) of the cross-correlation peak video, in pixels. Size is independent of sampling (type: int).',		
		'correlation_size_y' : 'Size (y) of the cross-correlation peak video, in pixels. Size is independent of sampling (type: int).',		
		'peak_search_radius_x' : 'Peak search radius (x), in pixels (type: int).',			
		'peak_search_radius_y' : 'Peak search radius (y), in pixels (type: int).',		
		...
		'orientation' : 'Include orientation angles in refinement. It is recommended to leave this set to true (type: true/false).',			
		'orientation_switch' : 'Iteration when Orientation Angles is switched from true«»false (type: int).',		
		'azimuth' : 'Include tilt azimuth in refinement. It is recommended to leave this set to true (type: true/false).',			
		...
		'scale' : 'Include scale factors (magnification) in refinement. It is recommended to leave this set to false for the first few iterations and possibly set this to true for later iterations (type: true/false).',
		...
		'recon_map_size_x' : 'Size (x) of the reconstructed tomogram, in pixels (type: int).',		
		'recon_map_size_y' : 'Size (y) of the reconstructed tomogram, in pixels (type: int).',		
		'recon_map_size_z' : 'Size (z) of the reconstructed tomogram, in pixels (type: int).',
		'recon_lowpass' : 'Apply a lowpass filter to images before reconstruction by back-projection?<br><br>If you are planning on performing single particle tomography on the reculting tomogram, set this to False. If instead you are planning to use the tomogram for segmenting or visualization purposes, set this to True and set the lowpass filters to the resolution you are hoping to be able to segment from or visualize. (type: True/False).',
		...
		'image_file_type' : 'Filetype extension for images. Protomo supports CCP4, EM, FFF, IMAGIC, MRC, SPIDER, SUPRIM, and TIFF (type: string).',		
		...
		//'gui_coarse_align' : 'Check to perform coarse alignment in the same manner as is done in tomoalign-gui\'s \'Align All\' function that uses only nearest neighbor alignment. Gridsearch and Area Matching Parameters are ignored.',
		'gridsearch_limit' : 'Grid Search +-angle limit, in degrees, for Coarse Alignment. To do a translational alignment only, set Grid Search +-Angle Limit to 0 and set Grid Search Angle Step Size to 1 (type: float).',		
		'gridsearch_step' : 'Grid Search angle step size, in degrees, for Coarse Alignment (type: float).',
		'coarse_retry_align' : 'The number of times Coarse Alignment will be restarted if it fails. Some tilt-series alignments fail because the search area is too large. This is usually because one or more Raw Images are shifted too far (type: int).',
		'coarse_retry_shrink' : 'Each successive retry will shrink the Window Size of the previous attempt by this fraction until either the alignment completes or the number of allowed retries of met (Default value reduces Window Size by 10%). Each retry will restart the alignment on the previously failed image (type: float > 0 & < 1).',
		'refine_retry_align' : 'The number of times a Refinement will be restarted if it fails. Some tilt-series alignments fail because the search area is too large. This is usually because one or more Raw Images are shifted too far (type: int).',
		'refine_retry_shrink' : 'Each successive retry will shrink the Window Size of the previous attempt by this fraction until either the alignment completes or the number of allowed retries of met (Default value reduces Window Size by 10%). Each retry will restart the alignment on the previously failed image (type: float > 0 & < 1).',
		'retry' : 'The number of times the alignment will be restarted if it fails. Some tilt-series alignments fail because the search area is too large. This is usually because one or more Raw Images are shifted too far (type: int).',
		'retry_shrink' : 'Each successive retry will shrink the Window Size of the previous attempt by this fraction until either the alignment completes or the number of allowed retries of met (Default value reduces Window Size by 10%). Each retry will restart the alignment on the previously failed image (type: float > 0 & < 1).',
		...
		'keep_recons' : 'Keep premilinary reconstructions used for generating a reconstruction video (type: true/false).',
		...
		'recon_iter' : 'Refinement iteration used to make final reconstruction. Note: this permanently discards all results from iterations greater than the number chosen here (type: int).',
		'coarse_param_file' : 'Full path to Coarse Alignment protomo param file. It is recommended that select/include and exclude are removed or set to 0-999999 and 999999, respectively. pathlist, cachedir, and outdir will be altered per tiltseries.<br><br>All Coarse Alignment parameters not included in ths section will be taken from this param file (type: string).',
		'refine_param_file' : 'Full path to Refinement protomo param file. It is recommended that select/include and exclude are removed or set to 0-999999 and 999999, respectively. pathlist, cachedir, and outdir will be altered per tiltseries.<br><br>All Refinement parameters not included in ths section will be taken from this param file (type: string).',
		'recon_param_file' : 'Full path to Reconstruction protomo param file. It is recommended that select/include and exclude are removed or set to 0-999999 and 999999, respectively. pathlist, cachedir, outdir, map size, and map sampling will be altered per tiltseries.<br><br>All Reconstruction parameters not included in ths section will be taken from this param file (type: string).',
		'procs' : 'Number of processors to use for parallel Alignment and Reconstruction. Batch processing is embarrasingly parallel; each processor goes linearly through the steps that you choose above (Preparation, Coarse Alignment, Refinement, and Reconstruction). For highest efficiency, choose a number that evenly divides the number of tilt-series to be processed. This value is limited by the number of processors on the machine protomo2batch.py will be run on (type: int or \'all\').',
		'link_recon' : 'Hardlink the final normalized .mrc reconstruction file to a directory. Provide full path. If no directory is provided, the reconstruction can be found in the tilt-series output directory\'s \'out\' folder (type: string).',
		...
		'voltage' : 'Microscope voltage, in keV (type: int).',
		'cs' : 'Microscope spherical aberration, in millimeters. A value of 0 can be entered; it will be made slightly larger to prevent division by 0 in the CTF equations (type: float).',
		'amp_contrast' : 'The fraction of amplitude contrast. For Cryo-EM, it should be between 0.07 and 0.14 (type: float).',
