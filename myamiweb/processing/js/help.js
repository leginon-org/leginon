...
**/
	...
	'protomo' : {
		'session' : 'For example, session=14apr15c',
		'projectid' : 'This should be the same number as the \'exp\' number in your address bar',
		'tiltseries' : 'This is somehow related to the particular tiltseries recorded during a session',
		'commit' : 'Set to true if you want to commit this run to the database',
		'windowsize_x' : 'Pixels in x to use for region matching',		
		'windowsize_y' : 'Pixels in y to use for region matching',		
		'sampling' : 'Sampling rate of raw data',		
		'thickness' : 'Estimated thickness of unbinned specimen (in pixels)',		
		'cos_alpha' : 'Cosine of the highest tilt angle',		
		'lowpass_diameter_x' : 'Lowpass diameter (x) in fractions of nyquist',		
		'lowpass_diameter_y' : 'Lowpass diameter (y) in fractions of nyquist',		
		'lowpass_apod_x' : 'Lowpass apodization (x) for rectangular and ellipsoidal masks',		
		'lowpass_apod_y' : 'Lowpass apodization (y) for rectangular and ellipsoidal masks',		
		'highpass_diameter_x' : 'Highpass diameter (x) in fractions of nyquist',		
		'highpass_diameter_y' : 'Highpass diameter (y) in fractions of nyquist',		
		'highpass_apod_x' : 'Highpass apodization (x) for rectangular and ellipsoidal masks',		
		'highpass_apod_y' : 'Highpass apodization (y) for rectangular and ellipsoidal masks',		
		'correlation_mode' : 'Correlation mode, standard (xcf), mutual (mcf), phase only (pcf), phase doubled (dbl)',		
		'raw_path' : 'Full path to raw image files',		
		'iters' : 'Number of alignment and geometry refinement iterations',		
		'param' : 'Override other parameters and use an external paramfile. Provide full path',		
		'binning' : 'Enable/disable binning of raw images',		
		'preprocessing' : 'Enable/disable preprocessing of raw image files',		
		'select_images' : 'Select specific images in the tilt series',		
		'exclude_images' : 'Exclude images from the tilt series',		
		'border_width' : 'Width of area at the image edge to exclude from image statistics',		
		'clip_low' : 'Lower threshold specified as a multiple of the standard deviation',		
		'clip_high' : 'Upper threshold specified as a multiple of the standard deviation',		
		'gradient' : 'Enable/disable linear gradient subtraction for preprocessing masks',		
		'iter_gradient' : 'Iterate gradient subtraction once',		
		'filter' : 'Preprocessing filter. Options are "median" or "gauss"',		
		'kernel_x' : 'Filter window size (x)',		
		'kernel_y' : 'Filter window size (y)',		
		'window_area_fraction' : 'Fraction of extracted area that must lie within the source image. Real value between 0 and 1',		
		'mask_apod_x' : 'Apodization (x) for rectangular and ellipsoidal masks',		
		'mask_apod_y' : 'Apodization (y) for rectangular and ellipsoidal masks',		
		'mask_width' : 'Rectangular mask width',		
		'estimate_parameters' : 'Estimate geometric parameters instead of using stored values from previous cycle',		
		'max_correction' : 'Terminate alignment if correction exceeds specified value',		
		'correlation_size_x' : 'Size (x) of cross correlation peak image',		
		'correlation_size_y' : 'Size (y) of cross correlation peak image',		
		'peak_search_radius_x' : 'Peak search region (x)',		
		'peak_search_radius_y' : 'Peak search region (y)',		
		'orientation' : 'Include orientation angles in refinement',		
		'azimuth' : 'Include tilt azimuth in refinement',		
		'elevation' : 'Include tilt axis elevation in refinement',		
		'rotation' : 'Include in-plane rotations in refinement',		
		'logging' : 'Enable diagnostic terminal output',		
		'logging_level' : 'Increase verbosity of diagnostic output',		
		'map_size_x' : 'Size (x) of the reconstructed tomogram',		
		'map_size_y' : 'Size (y) of the reconstructed tomogram',		
		'map_size_z' : 'Size (z) of the reconstructed tomogram',		
		'filename_prefix' : 'Prefix for input and output files, with the exception of raw image files, which are specified in the geometry file',		
		'image_extension' : 'Filetype extension for images. Protomo supports CCP4, EM, FFF, IMAGIC, MRC, SPIDER, SUPRIM,and TIFF',		
		'cachedir' : 'Directory where cache files are stored',		
		'outdir' : 'Directory where Protomo output files are stored',		
		'gridsearch_limit' : 'Gridseach +-angle limit for coarse alignment. To do a translational alignment only set to 1 and set gridsearch_limit to 0',		
		'gridsearch_step' : 'Gridseach angle step size for coarse alignment',
	}		
	,		
