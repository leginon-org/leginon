...
var help = {
	'appion' : {
		...
		'tiltaxis' : 'The initial tilt axis may need to be set if the program is failing to find the correct angle, e.g., when the tilt axis is horizontal rather than vertical',
		...
		'ctfcorrect' : 'CTF correct the particles, there are currently four methods available, you can CTF correct the whole image and then box the particles or CTF correct the particles after boxing them. A Wiener filter improves the quality of the particles by both phase flipping and adjusting the amplitudes.',
		...
		'emanboxfiles' : 'EMAN box files e.g. /home/$USER/files/*.box<br><br>  NOTE: box files must have the SAME name as the corresponding mrc files in the database. For example, the box file for <b>0059ma.mrc</b> must be named <b>0059ma.box</b>',
		'appionpartfile' : 'A tab separed file with x_coord &lt;tab&gt; y_coord &lt;tab&gt; filename with all images in one file, same file as downloaded from the particle reports',
