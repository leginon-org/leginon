...
var help = {
	'appion' : {
		...
		'hpmask': ' Special high pass filter that completely masks away the low resolution particle scattering and then linearly increase until it is set to 100% passed through at a higher resolution.',
		'hpzero': 'high pass filter resolution limit below which are set to zero',
		'hpone': 'high pass filter resolution limit beyong which are completely passed through',
		'edge1': 'Canny edge parameters Blur Sigma, Craig\'s favorite 6, 10 or 12 ',
		'edge2': 'Canny edge parameters Edge Treshold (0.0-1.0) ',
		'rotblur': 'Rotational blur of power spectra before finding the edges. This is helpful for images with low constrast CTF. Typical value is 5-10 degrees',
		...
		'tomoregiontype': 'subtomogram center can be obtained from a particle selection run or a stack/substack of the particles, no selection if no runs exist of the type',
		...
		'protomosample' : 'Sample every x pixels in the alignment. Non-integer is acceptable but should not be smaller than 1.  Larger values result in faster and more error-tolerable run but lack the accuracy of the lower values.  The default is a reasonable next value based on that of the preceeding iteration',
		'protomoregion' : 'A region of each tilt image centered at the preceeding or reset alignment shift is cut out for projection alignment.  The larger the region, the slower and less error-tolerable the alignment but will be more accurate given good starting values.  The defaults are based on the preceeding iteration as the next reasonable value. The value is not allowed if the resulting region in pixels plus twice the maximum shift of the resetting iteration is larger than the image size',
		'protomoreset': 'The new alignment iteration is a refinement of the alignment results from the preceeding iteration. If a large correction need to be made at the either end of the tilt series, protomo alignment may give false results.  Such false results can not be corrected by further iteration without resetting back to reasonable values and In the above rotation angle graph of the preceeding iteration, such failures appear as random or sudden jumps in alignment rotation angles.',
		...
		'protomogoodcycle' : 'Reset the alignment shifts outside the image number range to those of this iteration. The rotation is reset to that of the closest accepted image in the preceeding iteration of the intended run, not this one',
...
/******* IMAGIC terms ********/
		...
		'Threed_Reconstruction' : '<font size=1>PARAMETER 1: Hamming window - A hamming window can be incorporated into the 3d filter to suppress high frequency compontents that cause artefacts such as ringing, streaking, etc.  The effect of a hamming filter is to smooth off the edge of the filter rather than to have a sharp cut-off.  <BR/><BR/>For Hamming Window = 1.  No smooth edge, but rather a sharp cut-off at the nyquist frequency (effectively a circular mask).  <BR/><BR/>For Hamming Window = 0.8, you get a smooth transition from the value "1" at 0.6 times the Nyquist frequency to a value "0" at the Nyquist frequency.  <BR/><BR/>For Hamming Window = 0.5 you get a very smooth transition from the value "1" at the frequency origin, to a value "0" at the Nyquist frequency.  The amplitudes at half the Nyquist frequency will be damped to 0.5 of their original values. <br><br> PARAMETER 2: Object size - The relative size of the object inside the 3D volume influences the shape of the exact filter. <BR/><BR/> Low values (e.g.  0.1) have given poor results in test simulations.  High values (e.g.  0.8) have given better results.  While they may include some noise, this should be eliminated during the automasking procedure.',
		'Automasking' : '<b> Modulation-based masking of the 3d-density </b><br><br> PARAMETER 1: Lower bound of bandpass parameter - A good starting poing is the smallest dimension of our object. You can use the formula 2 / (number of pixels for smallest dimension of object).  For example, if it spans 17 pixels, input 0.12 here (2 / 17 = 0.12). <br><br> PARAMETER 2: This is a noise-reducing low-pass filter - Because initial models are done at low resolution, this can be set pretty low (e.g.  0.4) <br><br> PARAMETER 3: Mask sharpness parameter - The low-pass parameter needs to be >= the low bound of the bandpass.  The higher this filter is set, the "finer / sharper" the mask will be.  Acceptable values from 0-1. <br><br> PARAMETER 4: Density thresholding - What the program wants from you is what percentage of the voxels (between 0 and 100) you expect the wanted mask to cover.  The program will then automatically generate a threshold value that leads to a 3D mask approximately covering the wanted percentage of the available voxels in the 3D volume. In test runs, 15 has worked well',
		...
		'center' : 'perform a centering operation against the total particle sum prior to multi-reference alignment',
