...
/******* IMAGIC terms ********/
		...
		'protomo_corr_mode' : 'Correlation mode, standard (xcf), mutual (mcf), phase only (pcf), phase doubled (dbl). Note: ccc values outputed during Coarse Alignment to the command window by Protomo are normalized to xcf ccc values; mcf, pcf, and dbl ccc values are not accurate (type: string).',
		'protomo_create_reconstruction' : 'Create a reconstruction after Coarse Alignment for depiction as a gif on the next page (type: true/false).',
		'protomo_keep_recons' : 'Keep premilinary reconstructions used for generating a reconstruction gif (type: true/false).',
		'protomo_gif_optimize' : 'Optimizes depiction gif for ~25% reduced file size with very minimal quality degradation. Eg. Default Depiction Parameters produce a 40MB gif when not optimized and a 30MB gif when optimized (type: true/false).',
		...
		'protomo_link' : 'Raw Images will either be symlinked to the output \'raw\' directory or copied. Use linking if your system allows to significantly reduce overhead and disk accessing.',
		...
		'depictionparam_info' : 'These parameters relate to the depiction of the tomogram and diagnostic gifs on the next page.',
		...
		'corrimageinfo_refine' : 'Correlation peaks for each iteration are displayed here for quick refinement evaluation. Well-aligned reconstructions produce a point-like correlation peak gif where the peak does not wander more than a few pixels. Excluded images are not included.<br><br>To view depicton and diagnostics for an individual iteration, click on the corresponding correlation peak image or title.',
...
**/
	...
	'protomo' : {
		...
		'r1_region_x' : 'Pixels in x to use for region matching for Round 1. Default: 75% Raw Image size. This number will be binned by the requested sampling before alignment (type: int).',
		'r2_region_x' : 'Pixels in x to use for region matching for Round 2. Default: 75% Raw Image size. This number will be binned by the requested sampling before alignment (type: int).',
		'r3_region_x' : 'Pixels in x to use for region matching for Round 3. Default: 75% Raw Image size. This number will be binned by the requested sampling before alignment (type: int).',
		'r4_region_x' : 'Pixels in x to use for region matching for Round 4. Default: 75% Raw Image size. This number will be binned by the requested sampling before alignment (type: int).',
		'r1_region_y' : 'Pixels in y to use for region matching for Round 1. Default: 75% Raw Image size. This number will be binned by the requested sampling before alignment (type: int).',
		'r2_region_y' : 'Pixels in y to use for region matching for Round 2. Default: 75% Raw Image size. This number will be binned by the requested sampling before alignment (type: int).',
		'r3_region_y' : 'Pixels in y to use for region matching for Round 3. Default: 75% Raw Image size. This number will be binned by the requested sampling before alignment (type: int).',
		'r4_region_y' : 'Pixels in y to use for region matching for Round 4. Default: 75% Raw Image size. This number will be binned by the requested sampling before alignment (type: int).',
		...
		'map_sampling' : 'Sampling rate of Raw Images for use in summary reconstructions. Protomo well first create a weighted back-projection 3D reconstruction using the Sampling value in the General Parameters section, then a gif will be made and resized to this value if it is greater than the first Sampling value. For unbinned reconstructions, make sure you have enough CPU time limit (type: real >= 1 and > Sampling).',
		...
		'r1_lowpass_diameter_x' : 'Lowpass filter (x) for Round 1, in angstroms. This will filter out frequencies higher than the chosen value (type: float).',			
		'r2_lowpass_diameter_x' : 'Lowpass filter (x) for Round 2, in angstroms. This will filter out frequencies higher than the chosen value (type: float).',			
		'r3_lowpass_diameter_x' : 'Lowpass filter (x) for Round 3, in angstroms. This will filter out frequencies higher than the chosen value (type: float).',			
		'r4_lowpass_diameter_x' : 'Lowpass filter (x) for Round 4, in angstroms. This will filter out frequencies higher than the chosen value (type: float).',		
		'r1_lowpass_diameter_y' : 'Lowpass filter (y) for Round 1, in angstroms. This will filter out frequencies higher than the chosen value (type: float).',		
		'r2_lowpass_diameter_y' : 'Lowpass filter (y) for Round 2, in angstroms. This will filter out frequencies higher than the chosen value (type: float).',		
		'r3_lowpass_diameter_y' : 'Lowpass filter (y) for Round 3, in angstroms. This will filter out frequencies higher than the chosen value (type: float).',		
		'r4_lowpass_diameter_y' : 'Lowpass filter (y) for Round 4, in angstroms. This will filter out frequencies higher than the chosen value (type: float).',	
		...
		'retry_shrink' : 'Each successive retry will shrink the Window Size of the previous attempt by this fraction until either the alignment completes or the number of allowed retries of met (Default value reduces Window Size by 10%). Each retry will restart the alignment on the previously failed image (type: float).',
		'create_reconstruction' : 'Create a reconstruction after Coarse Alignment for depiction as a gif on the next page. If set to \'false\', then the remaining Depiction Parameters will be ignored (type: true/false).',
		'keep_recons' : 'Keep premilinary reconstructions used for generating a reconstruction gif (type: true/false).',
		'gif_optimize' : 'Optimizes depiction gif for ~25% reduced file size with very minimal quality degradation. Eg. Default Depiction Parameters produce a 40MB gif when not optimized and a 30MB gif when optimized (type: true/false).',
		...
		'coarse_param_file' : 'Full path to Coarse Alignment protomo param file. It is recommended that select/include and exclude are removed or set to 0-999999 and 999999, respectively. pathlist, cachedir, and outdir will be altered per tiltseries (type: string).',
		'refine_param_file' : 'Full path to Refinement protomo param file. It is recommended that select/include and exclude are removed or set to 0-999999 and 999999, respectively. pathlist, cachedir, and outdir will be altered per tiltseries (type: string).',
		'recon_param_file' : 'Full path to Reconstruction protomo param file. It is recommended that select/include and exclude are removed or set to 0-999999 and 999999, respectively. pathlist, cachedir, outdir, map size, and map sampling will be altered per tiltseries (type: string).',
		'procs' : 'Number of processors to use for parallel Alignment and Reconstruction. Batch processing is embarrasingly parallel; each processor goes linearly through the Protomo Coarse Alignment, Refinement, and Reconstruction steps. For highest efficiency, choose a number that evenly divides the number of tilt series to be processed. This value is limited by the number of processors on the machine protomo2batch.py will be run on (type: int).',
