...
/******* IMAGIC terms ********/
		...
		'protomo_tiltseries': 'Select the tilt series number in the current session that corresponds to the tomogram to be processed. Protomo supports CCP4, EM, FFF, IMAGIC, MRC, SPIDER, SUPRIM, and TIFF filetypes.',
		...
		'protomo_corr_mode' : 'Correlation mode, standard (xcf), mutual (mcf), phase only (pcf), phase doubled (dbl). Note: ccc values outputed during Coarse Alignment to the command window by Protomo are normalized to xcf ccc values; mcf, pcf, and dbl ccc values are not normalized to 1 (type: string).',
		'protomo_create_tiltvids' : 'Create tilt series videos after alignments for depiction (type: true/false).',
		'protomo_create_reconstruction' : 'Create a reconstruction after Coarse Alignment for depiction as a video (type: true/false).',
		'protomo_create_reconstruction2' : 'Create reconstructions after alignments for depiction as a video on the next page (type: true/false).',
		...
		'protomo_firststep' : 'This is the first processing step to be done in batch processing.',
		'protomo_laststep' : 'This is the last processing step to be done in batch processing.',
		...
		'screening_info' : 'Screening mode is intended to be run as tilt series are being collected. In screening mode the script will process all available tilt series in a session sequentially beginning with the first, then wait for more tilt series to appear and process those until the user manually stops the script at the command line. Screening mode will process tilt series through the Preparation and Coarse Alignment steps, generating correlation peak videos, initial tilt series videos, coarse aligned tilt series videos, and reconstruction videos. This may allow the user to determine whether a specimen will be alignable using Protomo and to determine optimal parameters (dose, defocus, tilt range, etc.) at Leginon before collecting an entire session.',
		'ctf_info' : 'Next Release!',
		'quality_assessment' : 'This plot shows the sum of the average correction factors and their standard deviations for each iteration. Generally lower is better.<br><br>Protomo refines geometric parameters by diagonalizing correction geometric matricies. This is done numerically, so the solution and thus the correction is not exact. To estimate the quality of a tilt series, the correction factors for x and y are averaged (with repect to 1.0; a Protomo convention) and the standard deviation of each is calculated. Plotted here is the sum of the average correction factors and the standard deviations. A candidate iteration for a well-aligned tilt series will have a value below 0.025.',
...
**/
	...
	'protomo' : {
		...
		'map_sampling' : 'Sampling rate of Raw Images for use in summary reconstructions. Protomo well first create a weighted back-projection 3D reconstruction using the Sampling value in the General Parameters section, then a video will be made and resized to this value if it is greater than the first Sampling value. For unbinned reconstructions, make sure you have enough CPU time limit (type: real >= 1 and > Sampling).',
		'thickness' : 'Estimated thickness of unbinned specimen in beam direction, in angstroms. \'Specimen\' refers to the object(s) near zero-tilt with the most contrast in the Raw Images. Protomo works best when this parameter is accurate to within 50% of the actual thickness. (type: int).',
		...
		'mask_apod_x' : 'Apodization (x) for rectangular and ellipsoidal masks, in pixels (type: float).',
		'mask_apod_y' : 'Apodization (y) for rectangular and ellipsoidal masks, in pixels (type: float).',
		...
		'r1_lowpass_diameter_x' : 'Lowpass filter (x) for Round 1, in angstroms. This will filter out frequencies higher than the chosen value.<br/><br/>This is a critical parameter. If you don\'t filter out frequencies higher than the highest resolvable structure of your specimen, Protomo might align to noise (type: float).',			
		'r2_lowpass_diameter_x' : 'Lowpass filter (x) for Round 2, in angstroms. This will filter out frequencies higher than the chosen value.<br/><br/>This is a critical parameter. If you don\'t filter out frequencies higher than the highest resolvable structure of your specimen, Protomo might align to noise (type: float).',			
		'r3_lowpass_diameter_x' : 'Lowpass filter (x) for Round 3, in angstroms. This will filter out frequencies higher than the chosen value.<br/><br/>This is a critical parameter. If you don\'t filter out frequencies higher than the highest resolvable structure of your specimen, Protomo might align to noise (type: float).',			
		'r4_lowpass_diameter_x' : 'Lowpass filter (x) for Round 4, in angstroms. This will filter out frequencies higher than the chosen value.<br/><br/>This is a critical parameter. If you don\'t filter out frequencies higher than the highest resolvable structure of your specimen, Protomo might align to noise (type: float).',		
		'r1_lowpass_diameter_y' : 'Lowpass filter (y) for Round 1, in angstroms. This will filter out frequencies higher than the chosen value.<br/><br/>This is a critical parameter. If you don\'t filter out frequencies higher than the highest resolvable structure of your specimen, Protomo might align to noise (type: float).',		
		'r2_lowpass_diameter_y' : 'Lowpass filter (y) for Round 2, in angstroms. This will filter out frequencies higher than the chosen value.<br/><br/>This is a critical parameter. If you don\'t filter out frequencies higher than the highest resolvable structure of your specimen, Protomo might align to noise (type: float).',		
		'r3_lowpass_diameter_y' : 'Lowpass filter (y) for Round 3, in angstroms. This will filter out frequencies higher than the chosen value.<br/><br/>This is a critical parameter. If you don\'t filter out frequencies higher than the highest resolvable structure of your specimen, Protomo might align to noise (type: float).',		
		'r4_lowpass_diameter_y' : 'Lowpass filter (y) for Round 4, in angstroms. This will filter out frequencies higher than the chosen value.<br/><br/>This is a critical parameter. If you don\'t filter out frequencies higher than the highest resolvable structure of your specimen, Protomo might align to noise (type: float).',	
		...
		'correlation_size_x' : 'Size (x) of cross correlation peak image, in pixels (type: int).',		
		'correlation_size_y' : 'Size (y) of cross correlation peak image, in pixels (type: int).',		
		...
		'map_size_x' : 'Size (x) of the reconstructed tomogram relative to Sampling, not to Sampling for Depiction (type: int).',		
		'map_size_y' : 'Size (y) of the reconstructed tomogram relative to Sampling, not to Sampling for Depiction (type: int).',		
		'map_size_z' : 'Size (z) of the reconstructed tomogram relative to Sampling, not to Sampling for Depiction (type: int).',		
		...
		'link_recons' : 'Hardlink the final normalized series*.mrc reconstruction files to a single directory. Provide full path (type: string).',
		...
		'restart_cycle' : 'Restart a Refinement at this iteration. Only use if a Refinement has previously been run.',
	},		
