...
// Create the tree
function createTree(arrName, startNode, openNodeArr) {
	nodes = arrName;
	if (nodes.length > 0) {
		preloadIcons();
		if (startNode == null) startNode = 0;
		if (openNodeArr != 0 || openNodeArr != null || openNodeArr.length>0)
			for (oni=0; oni<openNodeArr.length; oni++) {
				setOpenNodes(openNodeArr[oni]);
			}
		...
		} else document.write("<img src=\"img/base.gif\" align=\"absbottom\" alt=\"\" />Data<br />");
...
function createTreeold(arrName, startNode, openNode) {
	nodes = arrName;
	if (nodes.length > 0) {
		preloadIcons();
		if (startNode == null) startNode = 0;
...
if(!Array.prototype.remove) {
	function array_remove(item) {
	    ...
	    if (delindex>=0) {
	    	for (elementnr = delindex; elementnr<this.length;elementnr++)
