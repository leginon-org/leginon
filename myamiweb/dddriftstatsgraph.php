...
$topn=($_GET['top']) ? (int) $_GET['top'] : 5;
$offsetn=($_GET['offset']) ? (int) $_GET['offset'] : 0;

$data_name= ( $topn == 1 ) ? 'maximal movement':'first '.$topn.' movement average';
...
foreach ($imagestatsdata as $imagestats ) {
	...
	if (is_array($shift_data) && count($shift_data['positions']) > 1 ) {
		$shift_distances = $particle->getShiftDistancesFromPositions($shift_data['positions'],$topn+$offsetn);
		if ($topn < 2) {
		...
		$data[] = array('filename'=>$imagestats['filename'],'timestamp'=>$imagestats['timestamp'],'unix_timestamp'=>$imagestats['unix_timestamp'],$data_name=>$top_avg);
	}
	$counter += 1;
}
if ($viewsql) {
...
if ($viewdata) {
	$dbemgraph->dumpData(array('filename', 'timestamp', $data_name));
}
if ($histogram) {
