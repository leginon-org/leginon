...
If $before = 0 Then
   MsgBox($MB_OK, "Manual Beamstop", "Please retract beamstop")
   Exit(0)
EndIf
; activate the window
...
Local $status
$status = ControlClick($my_window, $my_text, 69572, "left", 1, 360, 12)
Sleep(500)
If $status <> 1 Then
   MsgBox($MB_OK, "Error", "Failed to click to retract beam stop")
EndIf
