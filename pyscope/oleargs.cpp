...
PyObject *PyCom_PyObjectFromSAFEARRAY(SAFEARRAY *psa, VARENUM vt /* = VT_VARIANT */)
{
	...
	int pDimension[MAX_DIMS];
	short HUGEP *pshort;
	//int *pDimension = NULL;
	//pDimension = new int[nDim];

	for(UINT i = 1; i <= nDim; i++)
	{
		long lb, ub;
		HRESULT hres = SafeArrayGetLBound(psa, i, &lb);
		if (FAILED(hres))
			return PyCom_BuildPyException(hres);
		hres = SafeArrayGetUBound(psa, i, &ub);
		if (FAILED(hres))
			return PyCom_BuildPyException(hres);
		pDimension[nDim-i] = ub - lb + 1;
	}

	if ((ret = (PyArrayObject *)PyArray_FromDims(nDim, pDimension, type)) == NULL)
		return FALSE;

	SafeArrayAccessData(psa, (void HUGEP* FAR*)&pshort);
	memcpy(ret->data, pshort, PyArray_Size((PyObject *)ret) * ret->descr->elsize);
	SafeArrayUnaccessData(psa);
