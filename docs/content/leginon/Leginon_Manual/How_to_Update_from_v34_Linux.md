...
## Download myami-3.5 source code

    git clone -b myami-3.5 http://emg.nysbc.org/git/myami myami-3.5
...
See [Configuring pyami.cfg for host-ip pairing](/leginon/Leginon_Manual/Start_Leginon/Test_Network_Connection_Between_Remote_and_Instrument_Computers/Configure_pyamicfg_for_host-ip_pairings)

# Upgrade web server

Rename your current myamiweb at the document root of the web server to something else as a backup.

## (optional) install the 3.5 version of pyami as it is done on processing server and restart redux server (The changes in pyami do not affect web server)
...
[How to Update from v3.4 (Instrument Computer) >](/leginon/Leginon_Manual/How_to_Update_from_v34_Linux/How_to_Update_from_v34_Instrument_Windows_Computer)
