...
class CentosInstallation(object):
	...
	def setReleaseDependantValues(self):
		...
		self.gitCmd = "git clone -b myami-3.4 http://emg.nysbc.org/git/myami " + self.gitMyamiDir
...
''' % (protomoDir, deplibs, deplibs))
	...
	def processServerYumInstall(self):
		...
		packagelist= ['epel-release','vim','wget','sudo','rsync','passwd','tar','firefox','mlocate','unzip','dbus-x11']
		self.yumInstall(packagelist)
