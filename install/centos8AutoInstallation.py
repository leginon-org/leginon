...
class CentosInstallation(object):
	...
	def yumInstall(self, packagelist):
		...
		cmd = "yum --skip-broken -y  install" + packagestr
		self.runCommand(cmd)
		#self.runCommand("updatedb")
	...
	def setupDBServer(self):
		...
		self.writeToLog("--- MariaDB is installed through yum")
...
''' % (protomoDir, deplibs, deplibs))
	...
	def processServerYumInstall(self):
		...
		packagelist = ['ImageMagick', 'fftw-devel', 'gcc', 'gcc-c++', 'gcc-gfortran', 'gcc-objc', 'gnuplot', 'grace', 'gsl-devel', 'libtiff-devel', 'netpbm-progs', 'openmpi-devel', 'xorg-x11-server-Xvfb', 'libjpeg-devel', 'zlib-devel', 'gtk3-devel', 'make', 'python39-devel']
		self.yumInstall(packagelist)
