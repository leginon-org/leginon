...
class LauncherSetupWidget(HostnamePortWidget):
	def __init__(self, parent, hostname=None, port=None):
		instructions = 'Please make sure \'Remote Leginon Server\' is running on the computer connected to the microscope. Then enter the hostname of the machine below and click \'Next\'.'

		if hostname is None:
			hostname = ''
		if port is None:
			port = defaultlauncherport
		HostnamePortWidget.__init__(self, parent, instructions, hostname, port)

#		self.portlabel.grid_configure(row=4)
#		self.portentry.grid_configure(row=4)
		...
		editportlabel = Tkinter.Label(self, text=editportinstructions,
																	wraplength=wraplength, justify=Tkinter.LEFT)
		editportlabel.grid(row=2, column=0, columnspan=2)

		self.editportvariable = Tkinter.IntVar()
		self.editportcheckbutton = Tkinter.Checkbutton(self, text='Specify port',
																							variable=self.editportvariable,
																							command=self.specifyPort)
		self.editportcheckbutton.grid(row=3, column=0, columnspan=2)

	def specifyPort(self):
		if self.editportvariable.get():
			self.portlabel.grid(row=4, column=0, sticky=Tkinter.E)
			self.portentry.grid(row=4, column=1, sticky=Tkinter.W)
		else:
			self.portlabel.grid_forget()
			self.portentry.grid_forget()
...
class SetupWizard(Wizard):
	...
	def initialSetup(self):
		self.setWidget(self.initialsetupwidget, None,
										self.initialSetupNext, self.destroy)

	def initialSetupNext(self):
		if self.initialsetupwidget.useremotevariable.get():
			self.launcherSetup()
		else:
			self.startSetup()
	...
	def startSetup(self):
		self.setWidget(self.startwidget, self.startBack,
										self.finishSetup, self.destroy)

	def finishSetup(self):
		launcher = self.launchersetupwidget.getHostnamePort()
		if launcher is None:
			launchers = []
		else:
			launchers = [launcher]
