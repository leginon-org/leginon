...
/* sfftw.h -- system-wide definitions */


 /* Swig wrapper file (modified sfftw.h and srfftw.h) by
...
%typemap(python,in) fftw_complex * {
  ...
  PyArrayObject *arr;
  if ((arr = (PyArrayObject *) PyArray_ContiguousFromObject($source, PyArray_CFLOAT, 0, 0)) == NULL) return NULL;
...
%typemap(python,in) fftw_real * {
  ...
  PyArrayObject *arr;
  if ((arr = (PyArrayObject *) PyArray_ContiguousFromObject($source, PyArray_FLOAT, 0, 0)) == NULL) return NULL;
