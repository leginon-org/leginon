...
class Prediction(object):
	...
	def newTiltSeries(self):							# potential problem when we abort a tilt series before adding groups. 
	...
	def predict(self, tilt):
		...
		if n_tilts < 1:
			...
			result = self.model(parameters, args_list)		# all predicted positions so far. 
			z0 = parameters[2]
			z = result[-1][-1][2] - z0
			x = result[-1][-1][0]
			y = result[-1][-1][1]
			...
			if n_tilts != n_start_fit:
				...
				if max(r2) > 0.95 and r2xy > 0.95 and not self.fixed_model:	# are our predictions so far on a linear line? 
