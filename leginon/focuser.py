...
class Focuser(manualfocuschecker.ManualFocusChecker):
	...
	def autoFocus(self, setting, emtarget, resultdata):
		...
		try:
			...
			correction = self.btcalclient.measureDefocusStig(btilt, correct_tilt=True, correlation_type=setting['correlation type'], stig=setting['stig correction'], settle=0.5, image0=lastdriftimage)
	...
	def avoidTargetAdjustment(self,target_to_adjust,recent_target):
		'''
		Avoid target adjustment when named as 'RCT Focus'. Refs #2665
		'''
