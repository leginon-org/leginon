...
class CameraLengthCalibrator(calibrator.Calibrator):
	...
	def calculateCameraLength(self):
		if self.shape is None:
			...
			return
		wavelength = fftfun.getElectronWavelength(self.instrument.tem.HighTension)
		d_spacing = self.settings['d spacing'] #Angstrum
		pixel_ring_diameter = self.settings['distance']*self.bin
		# camera physical pixel size
		meter_ring_diameter = pixel_ring_diameter*self.instrument.ccdcamera.getPixelSize()['x']
		theta = math.asin(wavelength*1e10/(2*d_spacing))
		camera_length = meter_ring_diameter/2.0/(math.tan(2*theta))
		self.image_camera_length = camera_length # meters
