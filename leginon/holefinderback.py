...
if __name__ == '__main__':
	...
	hf['original'] = mrc.read('/Users/acheng/Downloads/sdc1g22mar21f_a_00003gr_00008sq_v02_00016hl_v01.mrc')
	hf.threshold = 1.6
	template_file= os.path.join(leginon_dir,'hole_edge_template.mrc')
	hf.configure_edges(filter='sobel', size=None, sigma=3.0, absvalue=False, lpsig=1.0, thresh=30, edges=True)
	hf.configure_template(diameter=168, filename=template_file, filediameter=168, multiple=1, spacing=200.0,angle=25.0)
	hf.configure_correlation(cortype='cross', corfilt=(2.0,),cor_image_min=0)
	hf.configure_threshold(threshold=3.0, threshold_method="Threshold = mean + A * stdev")
	hf.configure_lattice(tolerance=0.1, spacing=415)
	print('saved test mrc imagings in current directory')
	hf.configure_ice(i0=1500, tmin=0.0)
	...
	print 'first holes2 of',len(hf['holes2']),hf['holes2'][0].stats,hf.configure_lattice(tolerance=0.08, spacing=122)
