...
def mrc_to_numeric(filename):
	mrc = numarray.memmap.open(filename, 'r')

	# get dimensions of image from header
	mrcdims_slice = mrc[:12]
	mrcdims = numarray.NumArray(buffer=mrcdims_slice, shape=(3,), type=numarray.Int32)
	c,r,s = mrcdims
	if s < 2:
		numshape = r,c
	else:
	...
	mrctype = numarray.NumArray(buffer=mrctype_slice, shape=(1,), type=numarray.Int32)
	mrctype = mrctype[0]
	numtype = mrcmode_type[mrctype][1]
	
	## !!! assuming this is 2-D MRC only
	mrcdata_slice = mrc[1024:]
	mrcdata = numarray.NumArray(buffer=mrcdata_slice, shape=numshape, type=numtype)
