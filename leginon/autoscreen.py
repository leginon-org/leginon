...
if __name__ == "__main__":
	answer = input('Enter autoloader cassette-grid mapping filename=')
	if answer:
		...
		comment_map = []
		ganswer = input('Grid loader slot number for the grid= ')
		if not ganswer:
			print('Error: Invalid grid loader slot number')
			sys.exit(1)
		answer = input('Session comment field entry: ')
		# grid slot, comment
		comment_map.append((int(ganswer),answer))
	# workflow choice
	wanswer = input('Full workflow or atlas only (full/atlas): ')
	while wanswer.lower() not in ('full','atlas'):
		wanswer = input('Invalid workflow name. Please try again: full or atlas')
