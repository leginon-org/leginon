...
# write relion 3.0 and 3.1 starfile output
# also exports to cryoSparc format
# wjr, NYU Langone, April-May 2019
# uodate 2/12/23: 
#                   # now works for cryosparc 4
#                   # will work for stacks uploaded to cryosparc which are not attached to mrc files
#                   # for cases where data was already grouped, puts Cs value for all equal to the most common Cs
#                   # also puts all other higher order CTF values to an equal number, thta of the first particle
...
#       matplotlib
#       scikit-learn
# datafile.py library from cryoSparc for cryoSparc export
        ...
        def readcspfile(self):
                try:
                        ...
                        for i, dataline in enumerate(self.stardata):
                                ...
                                filename = p4.sub('-a-DW.mrc',filename) # auto add DW to make it easier for users
                                groupdata[filename] = self.grouplabels[i]
                        keys = groupdata.keys()
                        if 'location/micrograph_path' in self.passthruset.data.keys():
                                pathkey = 'location/micrograph_path'
                        elif 'blob/path' in self.passthruset.data.keys():
                                pathkey = 'blob/path'
                        else:
                        ...
                        for i in range(len(self.particleset.data)):
                                filename = self.passthruset.data[pathkey][i]
                                ...
                                if basename in keys:
                                        ...
                                        numbad += 1
                        mode=0
                        # cs values need to be identical within groups but may have been modified by cryosparc
                        for cs_value in cs_values:
                                if cs_values[cs_value] > mode:
                                        cs_mode = cs_value
                                        mode = cs_values[cs_value]
                        for i in range(len(self.particleset.data)): # replace all cs values by most common value 
                                self.particleset.data['ctf/cs_mm'][i] = cs_mode
                                if i>0: # replace all other higher order ctfs with first particle value
                                        self.particleset.data['ctf/shift_A'][i]=self.particleset.data['ctf/shift_A'][0]
                                        self.particleset.data['ctf/tilt_A'][i]=self.particleset.data['ctf/tilt_A'][0]
                                        self.particleset.data['ctf/trefoil_A'][i]=self.particleset.data['ctf/trefoil_A'][0]
                                        self.particleset.data['ctf/tetra_A'][i]=self.particleset.data['ctf/tetra_A'][0]
                                        self.particleset.data['ctf/anisomag'][i]=self.particleset.data['ctf/anisomag'][0]
                        if (numbad >0):
                ...
                try:
                        ...
                        with f: 
                                ...
                                for dataline in self.stardata:
                                        ...
                                        name = p4.sub('-a-DW.mrc',name) # auto add DW to make it easier for users
