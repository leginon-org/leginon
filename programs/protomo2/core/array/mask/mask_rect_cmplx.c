/*----------------------------------------------------------------------------*
*
*  mask_rect_cmplx.c  -  array: mask operations
*
*-----------------------------------------------------------------------------*
*
...

{
  ...
  if ( ( param->apo == NULL ) || ( ~flags & MaskModeApod ) ) {
    ...
    switch ( dim ) {
      case 2: status = MaskRect2dCmplx( len, addr, A, b, param->wid, val, flags ); break;
      case 3: status = MaskRect3dCmplx( len, addr, A, b, param->wid, val, flags ); break;
    ...
    switch ( dim ) {
      case 2: status = MaskRectApod2dCmplx( len, addr, A, b, param->wid, param->apo, val, flags ); break;
      case 3: status = MaskRectApod3dCmplx( len, addr, A, b, param->wid, param->apo, val, flags ); break;
      default: status = exception( E_MASK_DIM );
    }
