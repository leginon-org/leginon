...
def runAce(matlab,img,params):
        ...
        acecommand=("ctfparams = ace('%s','%s',%d,%d,'%s',%e,'%s');" % (imgpath, params['outtextfile'], params['display'], params['stig'], 
...
#       print savematcommand
        ...
        if (params['stig']==0):
                ...
                print ('%1.3f   %1.3f   %1.3f       %1.3f       %1.3f' % (float(-nominal*1e6), float(ctfparams[0]*1e6), float(ctfparams[1]*1e6), 
ctfparams[16], ctfparams[17]))
