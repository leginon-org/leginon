...
def getAceValues(params,img):
	if params['hasace']=='TRUE':
		...
		imagedata=db.direct_query(dataclass=data.AcquisitionImageData, id=ctfparams[0]['imageId']['dbemdata|AcquisitionImageData|image'], readimages=False)
		...
		# if image & ctf data exist for filename
		if imagedata and ctfparams:
			conf_best=0
			params['apix']=getPixelSize(imagedata)
			params['kv']=(imagedata['scope']['high tension'])/1000
