...
from appionlib import apDDprocessv2,apDisplay,apDDprocess
...
class AppionCamFrameProcessingv2(apDDprocessv2.DDFrameProcessing):
        '''
        Appion upload frame movie processing
        '''
        def __init__(self,wait_for_new=False):
                super(AppionCamFrameProcessingv2,self).__init__(wait_for_new)
                self.setDefaultDimension(3838,3710)
                self.correct_dark_gain = True
                self.correct_frame_mask = False
        ...
        def _getRefImageData(self,reftype):
                imagedata = super(AppionCamFrameProcessingv2,self).getCorrectedImageData()
                if not self.use_full_raw_area:
                        refdata = imagedata[reftype]
                        return refdata
                else:
