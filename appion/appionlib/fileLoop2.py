...
class FileLoop(fileScript.FileScript):
	#=====================
	def __init__(self):
		"""
		Starts a new function and gets all the parameters
		overrides fileScript
		"""
		fileScript.FileScript.__init__(self)
		self.rundata = {}
	...
	def run(self):
		...
		while self.notdone:
			...
			while imgnum < len(self.imgtree) and self.notdone is True:
				...
				### set the pixel size
				self.params['apix'] = self.getPixelSize(imgdata)
...
# 					apDisplay.printMsg("New load average "+str(round(os.getloadavg()[0],2)))
	...
	def checkGlobalConflicts(self):
		...
		"""
		fileScript.FileScript.checkGlobalConflicts(self)
	...
	def setupGlobalParserOptions(self):
		...
		"""
		fileScript.FileScript.setupGlobalParserOptions(self)
	...
	def getPixelSize(self,imgdata):
		# fake pixel per angstrom
		return 1.0
	...
	def _startLoop(self, imgdata):
		...
		if self.params['parallel']:
			if self.lockParallel(imgdata.dbid):
				apDisplay.printMsg('%s locked by another parallel run in the rundir' % (imgdata['filename']))
				return False
		#calc images left
