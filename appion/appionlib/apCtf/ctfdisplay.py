...
class CtfDisplay(object):
    ...
    def extremaToIndex(self, requestVal, extremaList, pixelrdata):
        ...
        if False and self.debug is True:
            ...
            if extremaIndex < 100:
                print "... extremaList = ", numpy.array(extremaList, dtype=numpy.uint16)[
                    extremaIndex/2:extremaIndex*2]
            if extremaIndex > 0:
    ...
    def normalizeCtf(self, zdata2d, twod=True):
        ...
        while foundEnd is False:
            ...
            if valley[-1] < maxExtrema and numzeros < 1e5:
                ...
                apDisplay.printMsg(
                    "far from focus images, increasing number of extrema (%d)" % (numzeros))
                numzeros *= 2
                continue
        ...
        if distanceBetweenFinalPeaks > 2:
            if self.debug is True:
                apDisplay.printMsg("Low pass filter of %.1f pixels" %
                                   (distanceBetweenFinalPeaks/3.0))
        ...
        if len(peak) < 2:
            apDisplay.printWarning(
                "Too few peaks to work with (%d), probably bad defocus estimate" % (len(peak)))
            return None
        firstpeak = peak[0]
        ...
        numSections, noiseStartIndexes, noiseEndIndexes, mergeIndexes = self.createSections(
            numSections, fvi, indexData, pixelrdata, valley)
        ...
        if numSections == 1:
            ...
            for section in range(numSections-1):
                ...
                if self.debug is True:
                    ...
                    print "\tinsert from %d to %d (%d)" % (
                        insertStart, insertEnd, insertEnd-insertStart)
                    print "\toverlap from %d to %d (%d)" % (
        ...
        indexData = numpy.arange(0, len(raddata), dtype=numpy.float)
        numSections, envelopStartIndexes, envelopEndIndexes, mergeIndexes = self.createSections(
            numSections, fpi, indexData, pixelrdata, peak)
        ...
        self.overconf3010 = scipy.stats.pearsonr(
            normpeakdata[ind30:ind10], overctffitdata[ind30:ind10])[0]
        apDisplay.printColor("1/30A - 1/10A confidence is %.3f (overfocus %.3f)" %
                             (self.conf3010, self.overconf3010), "green")
        ...
        try:
            ...
            if len(peakradii) > 2:
                ...
                return None
        self.conf5peak = scipy.stats.pearsonr(
            normpeakdata[ind5peak1:ind5peak2], ctffitdata[ind5peak1:ind5peak2])[0]
        self.overconf5peak = scipy.stats.pearsonr(
            normpeakdata[ind5peak1:ind5peak2], overctffitdata[ind5peak1:ind5peak2])[0]
        apDisplay.printColor("5 peak confidence is %.3f (overfocus %.3f)" %
                             (self.conf5peak, self.overconf5peak), "green")
        ...
        pyplot.savefig(self.plotsfile, format="png", dpi=300,
                       orientation='landscape', pad_inches=0.0)
    ...
    def drawPowerSpecImage(self, origpowerspec, maxsize=1200):
        ...
        # merge information
        plotlimit2DAngstrom = (self.plotlimit2DAngstrom * self.res80 *
                               self.res50 * maxValleyResolution)**(1/4.)
        ...
        origpowerspec = ctftools.trimPowerSpectraToOuterResolution(
            origpowerspec, plotlimit2DAngstrom, self.trimfreq)
        ...
        halfpowerspec = numpy.hstack(
            (origpowerspec[:, :halfshape], ellipavgpowerspec[:, halfshape:]))
        ...
        for i in [-2, 2]:
            for j in [-2, 2]:
                draw.text((rightcoord[0]+i, rightcoord[1]+j),
                          "Elliptical Average", font=font, fill="black")
    ...
    def powerSpectra(self, image, ctfdata):
        ...
        subgenctfspec = ctftools.trimPowerSpectraToOuterResolution(
            genctfspec, self.plotlimit2DAngstrom, freq)
        ...
        powerspec = ctftools.trimPowerSpectraToOuterResolution(
            fullpowerspec, self.outerAngstrom1D, freq)
...
if __name__ == "__main__":
    ...
    imagelist.extend(
        glob.glob("/data01/leginon/10apr19a/rawdata/10apr19a_10apr19a_*23gr*10sq*02hl*17en_1.mrc"))
    ...
    imagelist.extend(
        glob.glob("/data01/leginon/12jun06h52a/rawdata/12jun06h52a_09oct22c*04sq*19hl*2en*.mrc"))
    ...
    for imgfile in imagelist:
        ...
        if os.path.isfile(powerspecfile):
            apDisplay.printColor("Skipping image %s, already complete" %
                                 (apDisplay.short(imagename)), "cyan")
            continue
        ...
        if ctfdata is None:
            apDisplay.printColor("Skipping image %s, no CTF data" %
                                 (apDisplay.short(imagename)), "red")
            continue
        #print ctfdata
        if ctfdata['confidence_30_10'] < 0.88:
            apDisplay.printColor("Skipping image %s, poor confidence" %
                                 (apDisplay.short(imagename)), "red")
            continue
        """
