	...
	try: #If anything fails, it's likely that something isn't in the path
		...
		if video_type == "gif":
			if align_step == "Coarse":
				...
				command2 += 'ffmpeg -y -v 0 -framerate 4.5 -pattery_type glob -i "%s" -codec:v libx264 -profile:v baseline -pix_fmt yuv420p -g 30 %s' % (png_full, mp4_full)
				...
				command2 += 'ffmpeg -y -v 0 -framerate 5.5 -pattery_type glob -i "%s" -codec:v libx264 -profile:v baseline -pix_fmt yuv420p -g 30 %s' % (png_full, mp4_full)
		else: #video_type == "html5vid"
			if align_step == "Coarse":
				command2 = "convert -delay 22 -loop 0 %s %s;" % (png_full, gif_full)
				command2 += 'ffmpeg -y -v 0 -framerate 4.5 -pattery_type glob -i "%s" -codec:v libtheora -b:v 3000K -g 30 %s;' % (png_full, ogv_full)
				command2 += 'ffmpeg -y -v 0 -framerate 4.5 -pattery_type glob -i "%s" -codec:v libx264 -profile:v baseline -pix_fmt yuv420p -g 30 %s;' % (png_full, mp4_full)
				command2 += 'ffmpeg -y -v 0 -framerate 4.5 -pattery_type glob -i "%s" -codec:v libvpx -b:v 3000K -g 30 %s' % (png_full, webm_full)
			else: #align_step == "Refinement"... Just changing the speed with the framerate option
				command2 = 'ffmpeg -y -v 0 -framerate 5.5 -pattery_type glob -i "%s" -codec:v libtheora -b:v 3000K -g 30 %s;' % (png_full, ogv_full)
				command2 += 'ffmpeg -y -v 0 -framerate 5.5 -pattery_type glob -i "%s" -codec:v libx264 -profile:v baseline -pix_fmt yuv420p -g 30 %s;' % (png_full, mp4_full)
				command2 += 'ffmpeg -y -v 0 -framerate 5.5 -pattery_type glob -i "%s" -codec:v libvpx -b:v 3000K -g 30 %s' % (png_full, webm_full)
		os.system(command2)
...
def makeReconstructionVideos(seriesname, iteraion, rundir, rx, ry, show_window_size, outdir, pixelsize, sampling, map_sampling, lowpass, thickness, video_type, keep_recons, parallel, align_step):
	...
	try: #If anything fails, it's likely that something isn't in the path
		...
		if video_type == "gif":
			...
			command += 'ffmpeg -y -v 0 -framerate 9 -pattery_type glob -i "%s" -codec:v libx264 -profile:v baseline -pix_fmt yuv420p -g 30 %s' % (png_full, mp4_full)
		else: #video_type == "html5vid"
			command = 'ffmpeg -y -v 0 -framerate 9 -pattery_type glob -i "%s" -codec:v libtheora -b:v 3000K -g 30 %s;' % (png_full, ogv_full)
			command += 'ffmpeg -y -v 0 -framerate 9 -pattery_type glob -i "%s" -codec:v libx264 -profile:v baseline -pix_fmt yuv420p -g 30 %s;' % (png_full, mp4_full)
			command += 'ffmpeg -y -v 0 -framerate 9 -pattery_type glob -i "%s" -codec:v libvpx -b:v 3000K -g 30 %s' % (png_full, webm_full)
		os.system(command)
		command2 = "rm %s" % (png_full)
		os.system(command2)
