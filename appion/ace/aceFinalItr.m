function   [ctfparams]= aceFinalItr(filename,outfile,display,stig,medium,dforig,tempdir, startOrientn, ang, rat); 
%
% DESCRIPTION: 
%
...
%   end 
  ...
  [val, pg] = getprofileItr(stig,medium,tempdir, startOrientn, pi, ang, rat);
...
%   x = xtemp-round(imwidth/50);
%    pgFlag=0;
%    while(x>1 && pg1(x)>pg1(x-1))       
%      x = x-1; pgFlag=1;
%    end 
%     while(x>1 && pgFlag && pg1(x)<pg1(x-1))
%      x = x-1; 
%     end 
% else
%     x = xtemp-round(imwidth/50);     
...
% x = find(pg1(1:imwidth/6)==max(pg1(1:imwidth/6)));
% while(x>1 && pg1(x+1)<pg1(x))
%     x = x+1;
% end
%  % if(~strcmp(medium,'ice'))
...
%  if startOrientn==0 | startOrientn==pi/2
%    figure; plot(pg1, '-'); hold on; plot(x,pg1(x), '*r'); title('pg1, pgram - 2nd iter');
%    plot(pgram, '-'); 
%  end
...
%  if startOrientn==0 | startOrientn==pi/2  
%    figure; plot(ctfenv, '-'); title(sprintf('%f ctfenv - 2nd iter', startOrientn));  
%  end
...
%   end 
  ...
  ctf = ctfenv./env; 
%   if startOrientn==0 | startOrientn==pi/2
%     figure; plot(ctf); title(sprintf('%f CTF - 2nd iter', startOrientn));  
%   end
...
%    if startOrientn==0 | startOrientn==pi/2
%         h=figure('Visible','off'); plot(ctffilt, '-'); hold on; plot(ctffinal, 'r-'); title(sprintf('%f degree - CTFFilt, CTFFinal - 2nd iter', startOrientn*180/pi));  
...
if(~isnan(conf) & conf ~= 0 & ~isnan(conf_d) & conf_d ~=0)
  ...
  fprintf(outid,'\n %s %f %f %f %f %f %f  %f %f %f %f %d %d',filename,...
     abs(dforig)*1e6, zinit*1e6, zfinal*1e6,zsmall*1e6,Afinal, rat, ang,conf,conf_d, sx, XnUpcut); 
%   fprintf('\n %s %f %f %f %f %f %f  %f %f %f %f %d %d',filename,...
%      abs(dforig)*1e6, zinit*1e6, zfinal*1e6,zsmall*1e6,Afinal, rat, ang,conf,conf_d, sx, XnUpcut); 
