...
function load_listbox(handles)
  ...
  if(get(handles.enr,'Value'))
    % using file filter, get list of all mrc files
    filefilter = get(handles.enr_filter,'String'); 
    dir_struct = dir(strcat(dirname,filefilter));
    list = [list; {dir_struct.name}'];
  end

  % get path to the matfiles directory
  mfiledir = get(handles.parentdir,'String'); % Directory for mat files
  expname = get(handles.expname,'String'); % Experiment name 
  matname = get(handles.matfilesdir,'String'); % Matfile dir name
  matfile_dir_path = strcat(mfiledir,'/',expname,'/',matname,'/');

  % for each file in the list, see if there is an existing matfile
  size_of_en_list=size(list);
  newlist={};
  newindxlist=0;
  j=1;
  for i=1:size_of_en_list
    matfile_name=strcat(matfile_dir_path,list{i},'.mat');
    if (exist (matfile_name)==0)
      newlist{1,j}=list{i};
      newindxlist(1,j)=i;
      j=j+1;
    end  
  end
  set(handles.filelistbox,'String',newlist,'thisIsTmp',newlistindxlist,'Value',[]);
