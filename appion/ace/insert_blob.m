function insert_blob(connect,tableName,fieldNames,data)
% This is a modified version of toolbox/database/database/@database/insert.m
% to support BLOB insert 
%
% Copyright Aug-16-2004 Denis Fellmann
% 
...
%   $Revision: 1.1 $	$Date: 2006-07-19 18:16:40 $%
...
for i = 1:numberOfRows,  
  ...
  for j = 1:cols,
    ...
    hexval=0;
    if isstrprop(tmp, 'xdigit') & isstr(tmp)
	hexval=1;
    end 
    
    %original
    %if isa(tmp,'double') || isa(tmp,'logical') %Test for data type.
    if isa(tmp,'double') || isa(tmp,'logical') || hexval 
      ...
      if (isempty(tmp) & isempty(nnw)) | (~isempty(nnw) & ~isempty(tmp) & tmp == nnw) | (isnan(tmp) & isnan(nnw)) 
          tmpstr = 'NULL';
      elseif hexval
          % to insert a BLOB '0x' as to be added
          % before the hexadecimal string.
          tmpstr = [ '0x' tmp ];
