function   ctfparams = ace(filename,outfile,display,stig,medium,dforig,tempdir); 
%
% DESCRIPTION: 
%
...
if(display & length(ind)>0 ) 
  ...
  plot([0:round(imwidth)/2-1]*1e-10/freqfactor,pg1.^2); 
  hold on;
  xmat = ([x x]-pixeloff)*1e-10/freqfactor;
  ymat = [0 max(pg1.^2)];
  plot(xmat,ymat,'k--'); 
  x11 = x+upcut;
  x1mat = ([x11 x11]-pixeloff)*1e-10/freqfactor;
  plot(x1mat,ymat,'k--'); 
  plot(snew,pgram.^2,'b');
  plot(snew,exp(b_calc1),'r-.'); 
  plot(snew,exp(b_calc1)+exp(b_calc2),'g--' ); 
  ylabel('Power Spectrum','fontsize',fntsz,'fontweight',fntwt); 
  xlabel('Frequency (1/A^o)','fontsize',fntsz,'fontweight',fntwt); 
  hfig = gca; 
  ...
  %plot(snew,exp(b_calc1)+exp(b_calc2),'g--' ); 
  hfig = gca; 
  ylabel('Noise Spectrum','fontsize',fntsz,'fontweight',fntwt); 
  xlabel('Frequency (1/A^o)','fontsize',fntsz,'fontweight',fntwt); 
  ...
  hold on; 
  plot(snew,ctfenv,'b') ; 
  ylabel('Envelope^2','fontsize',fntsz,'fontweight',fntwt); 
  xlabel('Frequency (1/A^o)','fontsize',fntsz,'fontweight',fntwt); 
  ...
  plot(snew,ctffilt,'b'); 
  %plot(snew(end-length(ctffilt)+1:end),ctffilt); 
  hold on; 
  plot(sind,ctffilt(ind),'r*'); 
  plot( snew, ctfinit,'r--');
  %title('CTF and Initial CTF'); 
  ylabel('CTF^2 (crude defocus)','fontsize',fntsz,'fontweight',fntwt); 
  xlabel('Frequency (1/A^o)','fontsize',fntsz,'fontweight',fntwt); 
  ...
  hold on; 
  plot(sind,ctffilt(ind),'r*'); 
  plot( snew,ctffinal,'g--');
  %title('CTF and Final CTF'); 
  ylabel('CTF^2 (refined defocus)','fontsize',fntsz,'fontweight',fntwt); 
  xlabel('Frequency (1/A^o)','fontsize',fntsz,'fontweight',fntwt); 
