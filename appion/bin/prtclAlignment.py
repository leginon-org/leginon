...
from appionlib.apImagicFile import readImagic, writeImagic
...
def align_particle(polar_refs, particle):
...
def align_stack(ref_stack, parts, outfile):
    parts_c = center_stack(parts)
    polar_refs = numpy.stack([warp_polar(ref_stack[i]) for i in range(len(ref_stack))])
    apDisplay.printMsg("Aligning particles to class averages")
    aligned = Parallel(n_jobs=-1)(
        delayed(align_particle)(polar_refs, parts_c[i]) for i in tqdm(range(len(parts)))
    )
    aligned = numpy.stack(aligned)
    apFile.removeStack(outfile)
    writeImagic(aligned, outfile)
...
def process_stack(stack):
    ...
    processed = numpy.stack([process_image(stack[i], mask) for i in tqdm(range(len(stack)))])    
    return processed
...
def stack_self_sort(stack, order_list=False):
    ...
    sorted_stack = numpy.stack(
        [a[1] for a in sorted(error_sums, key=lambda l: l[2])]
    )
    if order_list:
...
class TopologyRepScript(appionScript.AppionScript):
    ...
    def insertRunIntoDatabase(self):
        ...
        alignstackq["refstackfile"] = (
            os.path.basename(self.params["currentcls"]) + ".hed"
        )
    ...
    def alignClasses(self):
        ...
        if self.params["mramethod"] == "imagic":
            ...
            if self.params["currentiter"] < self.params["iter"]:
                if self.params["classiter"] is True:
                    ...
                    if (
                        self.params["nocenter"] is False
                        and self.params["nomask"] is False
                    ):
                        emancmd = "proc2d %s.hed %s_cen.hed center" % (
                            classname,
                            classname,
                        )
                        ## try cenalignint
                    ...
                    alifile = imagicAlignment.alirefs(
                        classname, mask=0.99, maxshift=0.1
                    )
                    if self.params["keepall"] is False:
                        apFile.removeStack(classname)
                    classname = alifile
            ...
            apDisplay.printColor(
                "finished alignment in "
                + apDisplay.timeString(time.time() - aligntime0),
                "cyan",
            )
            ...
            # mask the averages
            emancmd = "proc2d %s.hed %s_mask.hed mask=%i" % (
                classname,
                classname,
                (self.workingmask),
            )
        ...
        outputcls = os.path.join(
            self.params["rundir"], "classes%02i" % self.params["currentiter"]
        )
    ...
    def alignClassAvgs(self):
        classname = "classes_avg"
        # 1 Align 2 mask 3 normalize
        # todo: add centering in align, add masks and normalization
        if self.params["currentiter"] < self.params["iter"]:
            unaligned = readImagic(classname)["images"]
            processed = process_stack(unaligned)
            aligned = stack_organize(processed)
            apFile.removeStack(classname)
            writeImagic(aligned, classname)

        outputcls = os.path.join(
            self.params["rundir"], "classes%02i" % self.params["currentiter"]
        )
        # copy particles to run directory
        for ext in [".hed", ".img"]:
            cmd = f"scp {classname}{ext} {outputcls}{ext}"
            os.system(cmd)
        apFile.removeStack(classname)
    ...
    def runEMANmra(self):
        ...
        emancmd = (
            "classesbymra %s %s.hed mask=%i split imask=-1 logit=1 sep=1 phase"
            % (self.params["localstack"], self.params["currentcls"], self.workingmask)
        )
        ...
        self.params["alignedstack"] = string.replace(
            self.params["alignedstack"], "/jetstor/APPION", ""
        )
    ...
    def runIMAGICmra(self):
        ...
        if self.params["nproc"] > 1:
            f.write(
                "mpirun -np %i -x IMAGIC_BATCH %s/align/mralign.e_mpi << EOF\n"
        ...
        apDisplay.printColor(
            "Running IMAGIC .batch file: %s" % (os.path.abspath(bfile)), "cyan"
        )
        ...
        apDisplay.printColor(
            "finished IMAGIC in " + apDisplay.timeString(time.time() - aligntime0),
            "cyan",
        )
    ...
    def runIMAGICmsa(self):
        ...
        if self.params["msaproc"] > 1:
            f.write(
                "mpirun -np %i -x IMAGIC_BATCH %s/msa/msa.e_mpi << EOF\n"
    ...
    def readPartIMAGICFile(self):
        ...
        if not os.path.isfile(alifile):
            apDisplay.printWarning(
                "No IMAGIC alignment file found!  Setting alignment params to 0"
            )
    ...
    def sortFinalAverages(self):
        ...
        out = "sortedcls.hed"
        # read class averages
        unaligned = readImagic(self.params["currentcls"])["images"]

        # center, mask, and normalize particles 
        processed = process_stack(unaligned)

        self.sortedList, sorted_stack = stack_self_sort(processed, order_list=True)
        aligned = stack_self_align(sorted_stack)
        final_stack = process_stack(aligned)
        ...
        writeImagic(final_stack, self.params["currentcls"] + ".hed")
    ...
    def applySort(self):
        out = "classes_avg.hed"
        # read class averages
        avgfile = os.path.join(self.params["iterdir"], "classes.img")
        apFile.moveStack(avgfile, out)
    ...
    def start(self):
        ...
        self.params["localstack"] = os.path.join(
            self.params["rundir"], self.timestamp + ".hed"
        )
        ...
        if self.params["premask"] is True and self.params["mramethod"] == "imagic":
            ...
            shutil.move(
                maskstack + ".hed",
                os.path.splitext(self.params["localstack"])[0] + ".hed",
            )
            shutil.move(
                maskstack + ".img",
                os.path.splitext(self.params["localstack"])[0] + ".img",
            )
        ...
        if self.params["uploadonly"] is not True:
            ...
            for i in range(0, self.params["iter"] + 1):
                ...
                if i == 0:
                    ...
                    # print(self.params['alignedstack'])
                    self.params["alignedstack"] = os.path.splitext(
                ...
                #     self.runIMAGICmra()
                # else:
                # read stack and class files

                align_stack(
                    readImagic(self.params["currentcls"])["images"],
                    readImagic(self.params["alignedstack"])["images"],
        ...
        ## set upload information params:
            ...
            self.params["alignedstack"] = os.path.abspath("mrastack")
            if os.path.isfile(
                os.path.join(self.params["rundir"], self.params["currentcls"] + ".hed")
            ):
