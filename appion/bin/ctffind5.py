#!/gpfs/data/cryoem/software/anaconda2/bin/python
...
# 1.000000 14380.079102 14146.207031 67.006456 0.000000 0.050019 7.130980 260.724121 6.030597 1478.704346
		...
		os.remove(inputparams['output'] )  #delete the mrc file
		...
		os.remove(avgrotfile)   #no need to keep this log file
		
		os.remove(inputparams['input'] )  #delete the symlink mrc file
	...
	def convertCtffind4Resolution(self,res_str):
		...
		if res_float == float('inf'):
			...
			res_float = 100000.0
		if res_float > 50.0:
			res_float = 50.0  # a more reasonable cap
		return res_float

	#======================
	def commitToDatabase(self, imgdata):
		acerun = self.params['acerun']       # whether nor not to run ACE - now added as option in gui and command line
		self.insertCtfRun(imgdata)
		ctfinsert.validateAndInsertCTFData(imgdata, self.ctfvalues, self.ctfrun, self.params['rundir'],acerun)
