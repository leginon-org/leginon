...
class dBAutoMasker(appionLoop2.AppionLoop):
	...
	def setupParserOptions(self):
		...
		self.parser.add_option( "--epsilon", dest="epsilon", type="int", default=15,
			help="Epsilon for DBSCAN", metavar="#")
		self.parser.add_option("--min_samp", dest="min_samp", type="int", default=10,
			help="Minimum samples per cluster", metavar="#")
		self.parser.add_option("--perc_threshold", dest="perc_threshold", type="int", default=99,
			help="Threshold for initial binary mask", metavar="#")
...
#			apDatabase.insertImgAssessmentStatus(imgdata, 'run1', self.assess)
	...
	def preLoopFunctions(self):
		...
		os.system("pwd")	
		os.system("echo 'which shell?'; echo $0")
		os.system("echo 'dbmask_activate?';echo $DBMASK_ACTIVATE")
#		os.system("export DBMASK_ACTIVATE=/gpfs/sw/dbmask/env/bin")
		if os.environ.has_key('DBMASK_ACTIVATE'):
			self.activatepath = os.path.join(os.environ['DBMASK_ACTIVATE'], 'activate')
		else:
			apDisplay.printError("The environment variable 'DBMASK_ACTIVATE' is not set. This is the path to the activate program which sets up the environment to run dbmask. It should be 'path/to/dbmask/env/bin'. For more info see http://emg.nysbc.org/redmine/projects/appion/wiki/Install_EM_Hole_Finder or http://emg.nysbc.org/redmine/projects/appion/wiki/Install_DBMask.")
...
#		imgshape = numpy.asarray(imgdata['image'].shape)
		imgshape = numpy.asarray(self.image.shape)
		...
		epsilon    = str(self.params['epsilon'])
		min_samp = str(self.params['min_samp'])
		perc_threshold    = str(self.params['perc_threshold'])
		options = " --epsilon=" +  epsilon + " --min_samp=" + min_samp + " --perc_threshold=" + perc_threshold;
		
		commandline = ( "source " + self.activatepath + "; python `which DBMask.py` --ifile=" + jpg_image + " --ofile=" + self.outfile + options + "\n" )
		# Test with test image
		#commandline = ( "source /opt/em_hole_finder/env/bin/activate; python /opt/em_hole_finder/find_mask_amber.py \n" )

		### run command
		apDisplay.printMsg("Running dBMask " + time.asctime())
