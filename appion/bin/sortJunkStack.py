...
class sortJunkStackScript(appionScript.AppionScript):
	...
	def setupParserOptions(self):
		...
		self.parser.add_option("--numpart", dest="numpart", type="int",
			help="Limit particles for testing", metavar="STR")
	...
	def start(self):
		...
		apXmipp.breakupStackIntoSingleFiles(fn_oldstack, numpart=self.params['numpart'])
	...
	def uploadResults(self):
		# Get the new particle file order
		...
		for line in fh:
			...
			if (len(args)>1):
				...
				match = re.match('[A-Za-z]+([0-9]+)\.[A-Za-z]+', filename)
				if (match):
